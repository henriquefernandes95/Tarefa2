...
function gen_robot() {
    ...
    var left_upper_leg = gen_rect(2, 4);
    left_upper_leg.name = "left_upper_leg";
    var left_lower_leg = gen_rect(1.5, 3.5);
    left_lower_leg.name = "lower_leg";
    var left_foot = gen_rect(2.5,0.5);
    left_foot.name = "left_foot";
    left_upper_leg.add(left_lower_leg);
    left_lower_leg.add(left_foot);
    left_foot.position.y = -1.5;
    left_foot.position.x = -1.5;
    left_lower_leg.position.y = -3;
    left_upper_leg.position.x = -1.2;
    left_upper_leg.position.y = -4.6;



    // right: upper leg, leg, foot
    var right_upper_leg = gen_rect(2, 4);
    right_upper_leg.name = "right_upper_leg";
    var right_lower_leg = gen_rect(1.5, 3.5);
    right_lower_leg.name = "lower_leg";
    var right_foot = gen_rect(-2.5,0.5);
    right_foot.name = "left_foot";
    right_upper_leg.add(right_lower_leg);
    right_lower_leg.add(right_foot);
    right_foot.position.y = -1.5;
    right_foot.position.x = 1.5;
    right_lower_leg.position.y = -3;
    right_upper_leg.position.x = 1.2;
    right_upper_leg.position.y = -4.6;

    //EYE
    var left_eye = gen_circle(0.5);
    left_eye.position.y = 0.5;
    left_eye.position.x = -1;
    var right_eye = gen_circle(0.5);
    right_eye.position.y = 0.5;
    right_eye.position.x = 1;
